...
void help(struct board *brd, struct scrdim *dim) {
    const char helps[][35] = {
        "F1:        Help",
        "F2:        Toggle Hard Mode",
        "h,j,k,l:   Move Cursor",
...
void blink(struct board *brd, struct point *pts, struct scrdim *dim) {
    int *x = &pts->points[0][0];
    int *y = &pts->points[0][1];
...
int check_corner(struct board *brd, struct point *pts) {
    ...
    for(int i=0; i<pts->length; i++) {
        ...
        if(!pts->points[i][0]) {
            ...
            if(!pts->points[i][1]) {
                c[0].val = i;
                c[0].pts = pts->points[i][2];
            }
            
            if(pts->points[i][1] == brd->s-1) {
                c[1].val = i;
                c[1].pts = pts->points[i][2];
            }
        }
        if(pts->points[i][0] == brd->s-1) {
            
            if(!pts->points[i][1]) {
                c[2].val = i;
                c[2].pts = pts->points[i][2];
            }
            
            if(pts->points[i][1] == brd->s-1) {
                c[3].val = i;
                c[3].pts = pts->points[i][2];
            }
        }
    }
...
void compute(struct board *brd, struct point *pts) {
    
    int corner = check_corner(brd, pts);
    
    if(-1<corner) {
        pts->points[0][0] = pts->points[corner][0];
        pts->points[0][1] = pts->points[corner][1];
    }
    else {
        ...
        pts->points[0][0] = pts->points[index][0];
        pts->points[0][1] = pts->points[index][1];
        pts->points[0][2] = pts->points[index][2];
    }
